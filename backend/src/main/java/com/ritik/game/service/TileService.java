...
public class TileService {
    ...
    public TileDto captureTile(int tileId, UUID userId) {
        ...
        redisTemplate.convertAndSend("tile:updated", toJson(dto));

        return dto;
    }

    private String toJson(TileDto dto) {
        try {
            return objectMapper.writeValueAsString(dto);
        } catch (JsonProcessingException e) {
            throw new IllegalStateException("Failed to serialize tile update", e);
        }
    }
